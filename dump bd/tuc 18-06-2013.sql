...
-- Généré le : Mar 18 Juin 2013 à 12:48
...
--
-- Doublure de structure pour la vue `form_vVoteCounts`
--
DROP VIEW IF EXISTS `form_vVoteCounts`;
CREATE TABLE IF NOT EXISTS `form_vVoteCounts` (
`id` int(11)
,`clan_id` int(4)
,`defi_id` int(11)
,`pour` double
...
--
-- Doublure de structure pour la vue `vVoteContre`
--
DROP VIEW IF EXISTS `vVoteContre`;
CREATE TABLE IF NOT EXISTS `vVoteContre` (
`id` int(11)
,`plus` varchar(1)
,`moins` bigint(21)
);
...
--
-- Doublure de structure pour la vue `vVoteCount_part`
--
DROP VIEW IF EXISTS `vVoteCount_part`;
CREATE TABLE IF NOT EXISTS `vVoteCount_part` (
`id` int(11)
,`plus` varchar(21)
,`moins` varchar(20)
);
...
--
-- Doublure de structure pour la vue `vVotePour`
--
DROP VIEW IF EXISTS `vVotePour`;
CREATE TABLE IF NOT EXISTS `vVotePour` (
`id` int(11)
,`plus` bigint(21)
,`moins` varchar(1)
);
...
--
-- Doublure de structure pour la vue `vVotesDetails`
--
DROP VIEW IF EXISTS `vVotesDetails`;
CREATE TABLE IF NOT EXISTS `vVotesDetails` (
`user` int(11)
,`defi_clan` int(11)
,`type` int(11)
,`date_vote` date
...
--
-- Structure de la vue `form_vVoteCounts`
--
DROP TABLE IF EXISTS `form_vVoteCounts`;

CREATE ALGORITHM=UNDEFINED DEFINER=`tuc`@`%.mde.utc` SQL SECURITY DEFINER VIEW `form_vVoteCounts` AS select `v`.`id` AS `id`,`dc`.`clan_id` AS `clan_id`,`dc`.`defi_id` AS `defi_id`,sum(`v`.`plus`) AS `pour`,sum(`v`.`moins`) AS `contre` from (`vVoteCount_part` `v` join `form_defis_clans` `dc` on((`dc`.`id` = `v`.`id`)));

-- --------------------------------------------------------

--
-- Structure de la vue `vVoteContre`
--
DROP TABLE IF EXISTS `vVoteContre`;

CREATE ALGORITHM=UNDEFINED DEFINER=`tuc`@`%.mde.utc` SQL SECURITY DEFINER VIEW `vVoteContre` AS select `dc`.`id` AS `id`,'0' AS `plus`,count(0) AS `moins` from (`form_defis_clans` `dc` join `vVotesDetails` `v1` on((`v1`.`defi_clan` = `dc`.`id`))) where (`v1`.`type` = '1');

-- --------------------------------------------------------

--
-- Structure de la vue `vVoteCount_part`
--
DROP TABLE IF EXISTS `vVoteCount_part`;

CREATE ALGORITHM=UNDEFINED DEFINER=`tuc`@`%.mde.utc` SQL SECURITY DEFINER VIEW `vVoteCount_part` AS select `vVotePour`.`id` AS `id`,`vVotePour`.`plus` AS `plus`,`vVotePour`.`moins` AS `moins` from `vVotePour` union all select `vVoteContre`.`id` AS `id`,`vVoteContre`.`plus` AS `plus`,`vVoteContre`.`moins` AS `moins` from `vVoteContre`;

-- --------------------------------------------------------

--
-- Structure de la vue `vVotePour`
--
DROP TABLE IF EXISTS `vVotePour`;

CREATE ALGORITHM=UNDEFINED DEFINER=`tuc`@`%.mde.utc` SQL SECURITY DEFINER VIEW `vVotePour` AS select `dc`.`id` AS `id`,count(0) AS `plus`,'0' AS `moins` from (`form_defis_clans` `dc` join `vVotesDetails` `v1` on((`v1`.`defi_clan` = `dc`.`id`))) where (`v1`.`type` = '0');

-- --------------------------------------------------------

--
-- Structure de la vue `vVotesDetails`
--
DROP TABLE IF EXISTS `vVotesDetails`;

CREATE ALGORITHM=UNDEFINED DEFINER=`tuc`@`%.mde.utc` SQL SECURITY DEFINER VIEW `vVotesDetails` AS select `form_vote`.`user` AS `user`,`form_vote`.`defi_clan` AS `defi_clan`,`form_vote`.`type` AS `type`,`form_vote`.`date_vote` AS `date_vote` from `form_vote` union all select `form_vote_proxy`.`user` AS `user`,`form_vote_proxy`.`defi_clan` AS `defi_clan`,`form_vote_proxy`.`type` AS `type`,`form_vote_proxy`.`date_vote` AS `date_vote` from `form_vote_proxy`;
